...
revenue_param = sys.argv[1:]

with open('bakery.csv', 'r', encoding='utf-8') as file_show_sales:

    if len(revenue_param) == 0:
        start_index = 0
        stop_index = sum(1 for line in file_show_sales)
        file_show_sales.seek(0)
    elif len(revenue_param) == 1:
        start_index = int(revenue_param[0])
        stop_index = sum(1 for line in file_show_sales)
        file_show_sales.seek(0)
    elif len(revenue_param) == 2:
        start_index = int(revenue_param[0])
        stop_index = int(revenue_param[1])

    for index, line in enumerate(file_show_sales):
        if start_index - 1 <= index <= stop_index - 1:
            print(line.strip())
